...
export default function PostPage(){
    ...
    useEffect(() => {
        fetch(global.config.apiUrl+`/post/${id}`).then(
            response => {
    ...
    const handleDelete = () => {
        setIsDeleting(true);
        fetch(global.config.apiUrl+`/post/${id}`, {
          method: 'DELETE',
          headers: {
            ...
            if (response.ok) {
              // Post deleted successfully, navigate to a different page
              navigate('/'); // Replace '/' with the desired page after deletion
            } else {
    ...
    return(
        <div className="post-page">
            ...
            {userInfo.username_id === postInfo.author._id && (
